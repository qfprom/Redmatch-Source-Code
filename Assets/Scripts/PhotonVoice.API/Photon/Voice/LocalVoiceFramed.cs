...
namespace Photon.Voice
{
	public class LocalVoiceFramed<T> : LocalVoiceFramedBase
	{
		...
		public void PushDataAsync(T[] buf)
		{
			...
			{
				voiceClient.transport.LogInfo(base.LogPrefix + ": Starting data encode thread");
			...
			{
				voiceClient.transport.LogWarning(base.LogPrefix + ": PushData queue overflow. Frames skipped: " + (framesSkipped + 1));
		...
		private void PushDataAsyncThread()
		{
			...
			{
				voiceClient.transport.LogError(base.LogPrefix + ": Exception in encode thread: " + ex);
				throw ex;
			}
			finally
			{
				...
				voiceClient.transport.LogInfo(base.LogPrefix + ": Exiting data encode thread");
			}
		}
		...
		public void PushData(T[] buf)
		{
			...
			throw new Exception(base.LogPrefix + ": PushData(T[]) called on encoder of unsupported type " + ((encoder != null) ? encoder.GetType().ToString() : "null"));
		}
