...
namespace UnityEngine.Analytics
{
	...
	public class EventTrigger
	{
		...
		[SerializeField]
		private bool m_ApplyRules = false;

		[SerializeField]
		private TriggerListContainer m_Rules;

		[SerializeField]
		private TriggerBool m_TriggerBool = TriggerBool.All;
		...
		public bool Test(GameObject gameObject = null)
		{
			...
			foreach (TriggerRule rule in m_Rules.rules)
			{
				...
				{
					Debug.LogWarningFormat("Event trigger rule {0}{2} is incomplete ({1}). Result is false.", num2, message, (!(gameObject == null)) ? string.Format(" on GameObject '{0}'", gameObject.name) : null);
				}
