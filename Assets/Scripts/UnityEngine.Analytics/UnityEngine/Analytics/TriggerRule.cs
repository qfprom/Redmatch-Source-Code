...
namespace UnityEngine.Analytics
{
	...
	public class TriggerRule
	{
		...
		{
			bool result = false;
			double num = GetDouble(m_Value.propertyValue);
			switch (m_Operator)
			{
			case TriggerOperator.Equals:
				result = SafeEquals(currentValue, num);
				break;
			case TriggerOperator.DoesNotEqual:
				result = !SafeEquals(currentValue, num);
				break;
			...
			case TriggerOperator.IsGreaterThanOrEqualTo:
				result = currentValue > num || SafeEquals(currentValue, num);
				break;
			...
			{
				double num3 = GetDouble(m_Value2.propertyValue);
				result = currentValue > num && currentValue < num3;
				break;
			}
			case TriggerOperator.IsBetweenOrEqualTo:
			{
				double num2 = GetDouble(m_Value2.propertyValue);
				result = SafeEquals(currentValue, num) || SafeEquals(currentValue, num2) || (currentValue > num && currentValue < num2);
				break;
			}
			}
			return result;
