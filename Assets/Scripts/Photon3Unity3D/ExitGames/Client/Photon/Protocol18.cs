	...
	public class Protocol18 : IProtocol
	{
		...
		internal float ReadSingle(StreamBuffer stream)
		{
			...
			if (BitConverter.IsLittleEndian)
			{
				lock (memDouble)
				{
					memDouble[3] = bufferAndAdvance[offset++];
					memDouble[2] = bufferAndAdvance[offset++];
					memDouble[1] = bufferAndAdvance[offset++];
					memDouble[0] = bufferAndAdvance[offset];
				}
				return BitConverter.ToSingle(memDouble, 0);
			}
			return BitConverter.ToSingle(bufferAndAdvance, 0);
		}
		...
		internal double ReadDouble(StreamBuffer stream)
		{
			...
			if (BitConverter.IsLittleEndian)
			{
				lock (memDouble)
				{
					memDouble[7] = bufferAndAdvance[offset++];
					memDouble[6] = bufferAndAdvance[offset++];
					memDouble[5] = bufferAndAdvance[offset++];
					memDouble[4] = bufferAndAdvance[offset++];
					memDouble[3] = bufferAndAdvance[offset++];
					memDouble[2] = bufferAndAdvance[offset++];
					memDouble[1] = bufferAndAdvance[offset++];
					memDouble[0] = bufferAndAdvance[offset];
			...
			return BitConverter.ToDouble(bufferAndAdvance, 0);
		}
		...
		{
			byte b = 0;
			b = ((gpType != 0) ? ((byte)(gpType - 128)) : ((byte)stream.ReadByte()));
			CustomType value;
		...
		public override EventData DeserializeEventData(StreamBuffer din)
		{
			...
			eventData.Parameters = ReadParameterTable(din);
			return eventData;
		}
		...
		private Dictionary<byte, object> ReadParameterTable(StreamBuffer stream)
		{
			...
			for (int i = 0; i < num; i++)
			{
				...
				object value = Read(stream, (byte)stream.ReadByte());
		...
		private bool ReadDictionaryElements(StreamBuffer stream, GpType keyReadType, GpType valueReadType, IDictionary dictionary)
		{
			...
			for (int i = 0; i < num; i++)
			{
				object key = ((keyReadType == GpType.Unknown) ? Read(stream) : Read(stream, (byte)keyReadType));
		...
		private double[] ReadDoubleArray(StreamBuffer stream)
		{
			...
			Buffer.BlockCopy(array, 0, array2, 0, num * 8);
			return array2;
		}
		...
		internal void WriteDouble(StreamBuffer stream, double value, bool writeType)
		{
			...
			if (BitConverter.IsLittleEndian)
			{
				byte b = bufferAndAdvance[offset];
				byte b2 = bufferAndAdvance[offset + 1];
				byte b3 = bufferAndAdvance[offset + 2];
				byte b4 = bufferAndAdvance[offset + 3];
				bufferAndAdvance[offset] = bufferAndAdvance[offset + 7];
				bufferAndAdvance[offset + 1] = bufferAndAdvance[offset + 6];
				bufferAndAdvance[offset + 2] = bufferAndAdvance[offset + 5];
				bufferAndAdvance[offset + 3] = bufferAndAdvance[offset + 4];
				bufferAndAdvance[offset + 4] = b4;
				bufferAndAdvance[offset + 5] = b3;
				bufferAndAdvance[offset + 6] = b2;
				bufferAndAdvance[offset + 7] = b;
			}
		}
		...
		internal void WriteSingle(StreamBuffer stream, float value, bool writeType)
		{
			...
			if (BitConverter.IsLittleEndian)
			{
				byte b = bufferAndAdvance[offset];
				byte b2 = bufferAndAdvance[offset + 1];
				bufferAndAdvance[offset] = bufferAndAdvance[offset + 3];
				bufferAndAdvance[offset + 1] = bufferAndAdvance[offset + 2];
				bufferAndAdvance[offset + 2] = b2;
				bufferAndAdvance[offset + 3] = b;
			}
		}
		...
		internal void WriteString(StreamBuffer stream, string value, bool writeType)
		{
			...
			WriteIntLength(stream, bytes.Length);
		...
		internal void WriteSingleArray(StreamBuffer stream, float[] values, bool setType)
		{
			...
			stream.Write(array, 0, values.Length * 4);
		}
		...
		internal void WriteDoubleArray(StreamBuffer stream, double[] values, bool setType)
		{
			...
			stream.Write(array, 0, values.Length * 8);
		}
