...
namespace ExitGames.Client.Photon
{
	internal class EnetPeer : PeerBase
	{
		...
		internal override void InitPeerBase()
		{
			...
			if (photonPeer.DgramEncryptor != null && photonPeer.DgramDecryptor != null)
			{
		...
		internal void ApplyRandomizedSequenceNumbers()
		{
			...
			lock (channelArray)
			{
				...
				{
					int num = photonPeer.RandomizedSequenceNumbers[enetChannel.ChannelNumber % photonPeer.RandomizedSequenceNumbers.Length];
				...
				if (!base.IsSendingOnlyAcks && timeInt > timeoutInt && sentReliableCommands.Count > 0)
				{
					lock (sentReliableCommands)
					{
						...
						for (int i = 0; i < sentReliableCommands.Count; i++)
						{
							...
							if (nCommand.commandSentCount > photonPeer.SentCountAllowance || timeInt > nCommand.timeoutTime)
							{
								if ((int)base.debugOut >= 2)
								{
									base.Listener.DebugReturn(DebugLevel.WARNING, string.Concat("Timeout-disconnect! Command: ", nCommand, " now: ", timeInt, " challenge: ", Convert.ToString(challenge, 16)));
								}
								if (CommandLog != null)
								{
									CommandLog.Enqueue(new CmdLogSentReliable(nCommand, timeInt, roundTripTime, roundTripTimeVariance, true));
									CommandLogResize();
								}
						...
						while (commandsToResend.Count > 0)
						{
							...
							if ((int)base.debugOut >= 3)
							{
								base.Listener.DebugReturn(DebugLevel.INFO, string.Format("Resending: {0}. times out after: {1} sent: {3} now: {2} rtt/var: {4}/{5} last recv: {6}", nCommand2, nCommand2.roundTripTimeout, timeInt, nCommand2.commandSentTime, roundTripTime, roundTripTimeVariance, SupportClass.GetTickCount() - timestampOfLastReceive));
							}
						}
					}
				}
		...
		private void SendDataEncrypted(byte[] data, int length)
		{
			...
			photonPeer.DgramEncryptor.Encrypt(data, length, array, ref targetOffset);
			Buffer.BlockCopy(photonPeer.DgramEncryptor.FinishHMAC(array, 0, targetOffset), 0, array, targetOffset, HMAC_SIZE);
		...
		internal override void ReceiveIncomingCommands(byte[] inBuff, int dataLength)
		{
			...
			try
			{
				...
				if (b == 1)
				{
					if (photonPeer.DgramDecryptor == null)
					{
					...
					if (!photonPeer.DgramDecryptor.CheckHMAC(inBuff, dataLength))
					{
						packetLossByCrc++;
					...
					inBuff = photonPeer.DgramDecryptor.DecryptBufferWithIV(inBuff, offset, dataLength - offset - HMAC_SIZE, out dataLength);
				...
				for (int i = 0; i < b2; i++)
				{
					...
					if (nCommand.IsFlaggedReliable)
					{
						...
						{
							InReliableLog.Enqueue(new CmdLogReceivedReliable(nCommand, timeInt, roundTripTime, roundTripTimeVariance, timeInt - timeLastSendOutgoing, timeInt - timeLastSendAck));
							CommandLogResize();
						}
		...
		internal bool ExecuteCommand(NCommand command)
		{
			...
			case 15:
			{
				...
				byte[] array = new byte[fragment.totalLength];
				for (int num2 = fragment.startSequenceNumber; num2 < fragment.startSequenceNumber + fragment.fragmentCount; num2++)
				{
					NCommand fragment3;
					if (channel.TryGetFragment(num2, flag, out fragment3))
					{
						Buffer.BlockCopy(fragment3.Payload.GetBuffer(), 0, array, fragment3.fragmentOffset, fragment3.Payload.Length);
				...
				fragment.Payload = new StreamBuffer(array);
