...
namespace Photon.Pun.UtilityScripts
{
	public class PointedAtGameObjectInfo : MonoBehaviour
	{
		...
		{
			focus = ((!(pv != null)) ? null : pv.transform);
			if (pv != null)
			{
				text.text = string.Format("id {0} own: {1} {2}{3}", pv.ViewID, pv.OwnerActorNr, (!pv.IsSceneView) ? string.Empty : "scn", (!pv.IsMine) ? string.Empty : " mine");
			}
			else
			{
